...
- Resource polling is intentionally conditional: CPU/GPU resource stats run only while the system panel is visible, and fan stats only while the fan panel is visible.
- Keep `README.md` synchronized when changing documented packages, power behavior, installation steps, or Niri keybindings.

## Machine-Specific Assumptions

- Several settings are deliberately machine-specific: battery `BAT1`, display `eDP-1` with 2880x1800 modes, AMD GPU `card1`, and ASUS utilities/devices. Search all Niri, Quickshell, and systemd references before changing one of these assumptions.
